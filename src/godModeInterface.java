...
public class godModeInterface {
    ...
    public static void closeMarket(Connection connection) throws SQLException {
        ...
        try {
            // change the date in date table
            PreparedStatement setClosingPrice = connection.prepareStatement(
                    "UPDATE ClosingPrice C SET C.dailyClosingPrice FROM StarProfile S JOIN TimeInfo T ON S.stockSymbol = C.stockSymbol WHERE S.stockSymbol = C.stockSymbol AND T.currentDate = C.priceDate");
